...
export const optimizeImage = (options = {}) => {
    return async (req, res, next) => {
        try {
            ...
            const { width = 1920, height = 1080, quality = 80, format = 'jpeg', progressive = true } = options;
...
export const validateFile = (req, res, next) => {
    ...
    if (!SUPPORTED_IMAGE_TYPES.includes(file.mimetype)) {
        const error = createValidationError(`Unsupported file type: ${file.mimetype}. Supported types: ${SUPPORTED_IMAGE_TYPES.join(', ')}`);
        return next(error);
    }
    // Validate file size
    if (file.size > FILE_SIZE_LIMITS.image) {
        const error = createValidationError(`File too large: ${(file.size / 1024 / 1024).toFixed(2)}MB. Maximum allowed: ${FILE_SIZE_LIMITS.image / 1024 / 1024}MB`);
        return next(error);
    }
    ...
    if (!allowedExtensions.includes(fileExtension)) {
        const error = createValidationError(`Invalid file extension: ${fileExtension}. Allowed extensions: ${allowedExtensions.join(', ')}`);
        return next(error);
    }
    next();
};
...
export const validateMultipleFiles = (req, res, next) => {
    ...
    for (const file of files) {
        // Validate file type
        if (!SUPPORTED_IMAGE_TYPES.includes(file.mimetype)) {
            const error = createValidationError(`Unsupported file type: ${file.mimetype} in file ${file.originalname}`);
            return next(error);
        }
        // Validate file size
        if (file.size > FILE_SIZE_LIMITS.image) {
            const error = createValidationError(`File too large: ${file.originalname} (${(file.size / 1024 / 1024).toFixed(2)}MB)`);
            return next(error);
        }
    }
    next();
...
export const extractImageMetadata = async (req, res, next) => {
    try {
        if (!req.file) {
            return next();
        }
