...
class AzureStorageService {
    ...
    async initializeClient() {
        ...
        try {
            ...
            this.blobServiceClient = new BlobServiceClient(`https://${this.config.accountName}.blob.core.windows.net`, this.credential);
     ...
     */
    generateBlobName(originalName, userId) {
     ...
     */
    async uploadFile(containerName, file, fileName, userId, options = {}) {
     ...
     */
    async listFiles(containerName, prefix, maxResults) {
        await this.ensureInitialized();
        const containerClient = this.containers[containerName];
        const files = [];
        try {
     ...
     */
    async generateSignedUrl(containerName, blobName, options) {
        await this.ensureInitialized();
        const containerClient = this.containers[containerName];
        const blockBlobClient = containerClient.getBlockBlobClient(blobName);
        const permissions = new BlobSASPermissions();
        if (options.permissions.includes('r'))
            permissions.read = true;
        if (options.permissions.includes('w'))
            permissions.write = true;
        if (options.permissions.includes('d'))
     ...
     */
    async exportDeck(deckData, format = 'json') {
        await this.ensureInitialized();
        let content;
        let contentType;
        let fileName;
        ...
        return this.uploadFile('exports', buffer, fileName, deckData.exportedBy, {
            contentType,
            metadata: {
     ...
     */
    async importDeck(containerName, blobName) {
        await this.ensureInitialized();
     ...
     */
    parseDeckFromText(content, metadata) {
     ...
     */
    async cleanupOldFiles(containerName, olderThanDays) {
        await this.ensureInitialized();
        const containerClient = this.containers[containerName];
