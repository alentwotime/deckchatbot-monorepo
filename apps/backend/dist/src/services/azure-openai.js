...
class AzureOpenAIService {
    ...
    MTG_SYSTEM_PROMPT = `You are an expert Magic: The Gathering deck analyst and strategist. You have deep knowledge of:
...
Be thorough, accurate, and helpful. Use proper MTG terminology and consider the current meta when making recommendations.`;
     ...
     */
    async createStreamingChatCompletion(request, onChunk, onComplete, onError) {
        if (!this.openai) {
            throw new Error('Azure OpenAI not configured');
        }
        try {
...
Format your response as a detailed analysis with clear sections.`;
     ...
     */
    calculateCompetitiveRating(deckList, format) {
        ...
        if (format === 'Standard' && cardCount >= 60)
            rating += 1;
        if (format === 'Modern' && cardCount >= 60)
            rating += 1;
        if (format === 'Commander' && cardCount >= 100)
            rating += 1;
