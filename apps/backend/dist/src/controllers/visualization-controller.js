...
export class VisualizationController {
    ...
    generateDeckBlueprint = async (req, res) => {
        try {
            ...
            if (blueprint.format === 'svg' || blueprint.format === 'png') {
                const uploadResult = await this.storageService.uploadFile('blueprints', Buffer.from(blueprint.data), `blueprint_${blueprintRecord.id}.${blueprint.format}`, userId, {
    ...
    create3DDeckModel = async (req, res) => {
        try {
            ...
            if (model3D.format === 'gltf' || model3D.format === 'obj') {
                const uploadResult = await this.storageService.uploadFile('3d-models', Buffer.from(model3D.data), `model_${modelRecord.id}.${model3D.format}`, userId, {
     ...
     */
    processUploadedDrawing = async (req, res) => {
        try {
     ...
     */
    generateDeckStatisticsVisualization = async (req, res) => {
        try {
            ...
            if (visualization.format === 'svg' || visualization.format === 'png') {
                const uploadResult = await this.storageService.uploadFile('visualizations', Buffer.from(visualization.data), `viz_${vizRecord.id}.${visualization.format}`, userId, {
     ...
     */
    getVisualizationHistory = async (req, res) => {
        try {
     ...
     */
    deleteVisualization = async (req, res) => {
        try {
    ...
    // Helper methods
    async calculateComprehensiveStatistics(deck) {
        ...
        return {
            ...
            competitiveRating: await this.openAIService.calculateCompetitiveRating(cards.map((card) => `${card.quantity} ${card.name}`).join('\n'), deck.format)
        };
    }
    ...
    async generateDrawingVisualization(params) {
    ...
    async generateManaCurveVisualization(manaCurve, settings) {
    ...
    async generateColorDistributionVisualization(colorDist, settings) {
    ...
    async generateCardTypeVisualization(cardTypes, settings) {
    ...
    async generateRarityVisualization(rarityDist, settings) {
    ...
    async generateComprehensiveVisualization(statistics, settings) {
