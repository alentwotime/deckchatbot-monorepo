...
export const createCustomLimiter = (options = {}) => {
    const { windowMs = 15 * 60 * 1000, max = 100, keyGenerator = (req) => req.ip, message = 'Too many requests', skipSuccessfulRequests = false, skipFailedRequests = false } = options;
...
export const adaptiveLimiter = (baseOptions = {}) => {
    const { baseMax = 100, windowMs = 15 * 60 * 1000, loadThresholds = { low: 0.5, medium: 0.7, high: 0.9 } } = baseOptions;
    return async (req, res, next) => {
        try {
