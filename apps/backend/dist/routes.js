...
router.post('/upload-file', uploadLimiter, upload.single('file'), secureFileUpload, validateRequest({
    ...
    try {
        ...
        const fileRecord = await dbService.query(`INSERT INTO files (original_name, mime_type, size, file_type, file_path, uploaded_at) 
       VALUES ($1, $2, $3, $4, $5, NOW()) 
       RETURNING id, original_name, uploaded_at`, [originalname, mimetype, size, fileType, filename]);
        res.json({
            success: true,
            file: {
...
router.post('/analyze-files', dbLimiter, async (req, res) => {
    try {
        ...
        await dbService.query(`INSERT INTO analyses (file_ids, analysis_data, created_at) 
       VALUES ($1, $2, NOW())`, [fileIds, JSON.stringify(analysis)]);
        res.json({
            success: true,
            analysis,
            cached: false
...
router.post('/generate-blueprint', dbLimiter, async (req, res) => {
    try {
        ...
        await dbService.query(`INSERT INTO blueprints (blueprint_data, analysis_hash, created_at) 
       VALUES ($1, $2, NOW())`, [JSON.stringify(blueprint), analysisHash]);
        res.json({
            success: true,
            blueprint,
            cached: false
