...
class DataRetentionService {
  ...
  async cleanupExpiredFiles() {
    ...
    try {
      ...
      const filesToDelete = await dbService.query(
        `SELECT id, file_path, file_url FROM uploads WHERE uploaded_at < $1`,
        [cutoffDate]
      );
      ...
      if (filesToDelete.rows.length > 0) {
        const result = await dbService.query(
          `DELETE FROM uploads WHERE uploaded_at < $1`,
          [cutoffDate]
        );
    ...
    try {
      await dbService.transaction(async (client) => {
        ...
        const files = await client.query(
          `SELECT file_path FROM uploads u 
