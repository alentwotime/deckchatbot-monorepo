...
export const processingRateLimit = createRateLimitMiddleware(
  rateLimiters.processing,
  'Too many processing requests, please wait for current operations to complete.',
  'PROCESSING_RATE_LIMIT_EXCEEDED'
);

// Burst protection rate limiting
export const burstProtectionRateLimit = createRateLimitMiddleware(
  rateLimiters.burst,
...
export const heavyOperationRateLimit = createRateLimitMiddleware(
  rateLimiters.heavy,
  'Heavy operation limit exceeded. Please wait before performing another resource-intensive operation.',
  'HEAVY_OPERATION_RATE_LIMIT_EXCEEDED'
);

// Advanced user-based rate limiting with different tiers
export const tieredUserRateLimit = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const userTier = req.user?.tier || 'free';
    const rateLimiter = tieredRateLimiters[userTier as keyof typeof tieredRateLimiters] || tieredRateLimiters.free;
    ...
    if (rateLimiterRes instanceof RateLimiterRes) {
      ...
      const error = createRateLimitError(
        `Rate limit exceeded for ${userTier} tier. Limit: ${rateLimiter.points} requests per 15 minutes.`
      );
...
export const userBasedRateLimit = (
  ...
  return createRateLimitMiddleware(
    rateLimiter,
    errorMessage || `Too many requests. Limit: ${maxRequests} per ${windowSeconds} seconds.`,
...
export const clearUserRateLimit = async (userId: string): Promise<boolean> => {
  try {
    await Promise.all([
