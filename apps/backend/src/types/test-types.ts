...
export function testTypes(): void {
  ...
  const test3DModel: ThreeDModelConfig = {
    name: 'Test 3D Model',
    version: '1.0.0',
    format: 'gltf',
    geometry: {
  ...
  const testBlueprintSettings: BlueprintSettings = {
    grid: {
      enabled: true,
      size: 10,
      subdivisions: 10,
  ...
  // Test DeckBlueprint
  const testDeckBlueprint: DeckBlueprint = {
    userId: 'test-user',
    name: 'Test Blueprint',
  ...
  // Test OCR types
  const testOCRRequest: OCRRequest = {
 ...
 */
export function testVisualAndComputerVisionTypes(): void {
  console.log('🧪 Testing Visual Feature and Computer Vision types...');

  // Test ImageProcessingOptions
  const testImageProcessing: ImageProcessingOptions = {
    input: {
      format: 'auto',
      quality: 0.9,
      maxSize: {
  ...
  const testDrawingAnalysis: DrawingAnalysis = {
    input: {
      imageId: 'img-123',
  ...
  const testCardRecognition: CardRecognitionResponse = {
    cards: [
      {
        name: 'Lightning Bolt',
        confidence: 0.95,
